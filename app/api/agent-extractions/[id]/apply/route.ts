...
export async function POST(
  ...
  // Service client: a posse ja foi verificada acima, evita falsos negativos por RLS
  // no merge (mesmo padrao usado em call-pipeline.ts e leads/[id]/route.ts).
  const svc = createServiceClient()
  const result = await applyExtraction(svc, id)
