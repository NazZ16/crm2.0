...
import { PROPERTY_TYPE_LABELS, type Opportunity } from '@/lib/types'
...
const EMPTY_FORM: Record<string, string> = {
  ...
  source: '', description: '',
  estimated_monthly_rent: '', condo_fee: '0', annual_imi: '0', renovation_cost: '0',
  ...
  construction_cost: '0', operational_expenses: '0', renovation_item3: '0',
  renovation_item4: '0', renovation_item5: '0', estimated_sell_price: '',
  sale_commission_pct: '4', sale_commission2_pct: '0', sale_commission3_pct: '0',
  early_repayment_penalty_pct: '0', operator_profit_pct: '25', irc_rate: '19',
  reform_months: '10', contract_type: 'empreiteiro',
}
...
export default function EditOpportunityPage() {
  ...
  const [dealType, setDealType] = useState<'buy_to_let' | 'fix_and_flip'>('buy_to_let')
  ...
  const loadOpp = useCallback(async () => {
    ...
    setDealType(opp.deal_type ?? 'buy_to_let')
    setForm({
      title: str(opp.title),
      address: str(opp.address),
      zone: str(opp.zone),
      typology: str(opp.typology),
      property_type: str(opp.property_type) || 'apartment',
      asking_price: str(opp.asking_price),
      negotiated_price: str(opp.negotiated_price),
      area_m2: str(opp.area_m2),
      vpt: str(opp.vpt),
      source: str(opp.source),
      description: str(opp.description),
      estimated_monthly_rent: str(opp.estimated_monthly_rent),
      condo_fee: str(opp.condo_fee ?? 0),
      annual_imi: str(opp.annual_imi ?? 0),
      renovation_cost: str(opp.renovation_cost ?? 0),
      imposto_selo_pct: str(opp.imposto_selo_pct ?? 0.008),
      escritura_cost: str(opp.escritura_cost ?? 1000),
      financing_entry_pct: str(opp.financing_entry_pct ?? 100),
      financing_interest_pct: str(opp.financing_interest_pct ?? ''),
      financing_years: str(opp.financing_years ?? ''),
      financing_stamp_duty_pct: str(opp.financing_stamp_duty_pct ?? 0.006),
      financing_dossier: str(opp.financing_dossier ?? 0),
      financing_evaluation: str(opp.financing_evaluation ?? 0),
      financing_formalization: str(opp.financing_formalization ?? 0),
      financing_mortgage_registry: str(opp.financing_mortgage_registry ?? 0),
      holding_months: str(opp.holding_months ?? 12),
      insurance_monthly: str(opp.insurance_monthly ?? 0),
      electricity_monthly: str(opp.electricity_monthly ?? 0),
      water_monthly: str(opp.water_monthly ?? 0),
      construction_cost: str(opp.construction_cost ?? 0),
      operational_expenses: str(opp.operational_expenses ?? 0),
      renovation_item3: str(opp.renovation_item3 ?? 0),
      renovation_item4: str(opp.renovation_item4 ?? 0),
      renovation_item5: str(opp.renovation_item5 ?? 0),
      estimated_sell_price: str(opp.estimated_sell_price),
      sale_commission_pct: str(opp.sale_commission_pct ?? 4),
      sale_commission2_pct: str(opp.sale_commission2_pct ?? 0),
      sale_commission3_pct: str(opp.sale_commission3_pct ?? 0),
      early_repayment_penalty_pct: str(opp.early_repayment_penalty_pct ?? 0),
      operator_profit_pct: str(opp.operator_profit_pct ?? 25),
      irc_rate: str(opp.irc_rate ?? 19),
      reform_months: str(opp.reform_months ?? 10),
      contract_type: str(opp.contract_type) || 'empreiteiro',
    })
    setFetching(false)
  }, [id, router])
  ...
  const isFf = dealType === 'fix_and_flip'
  const btlPreview = useMemo(() => !isFf ? previewBtl(form) : null, [form, isFf])
  const ffPreview = useMemo(() => isFf ? previewFf(form) : null, [form, isFf])
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const body: Record<string, unknown> = {
      ...
      negotiated_price: num('negotiated_price') ?? null,
      area_m2: num('area_m2') ?? null,
      vpt: num('vpt') ?? null,
    ...
    if (!isFf) {
      ...
      body.financing_interest_pct = flt('financing_interest_pct') ?? null
      body.financing_years = num('financing_years') ?? null
      ...
      body.estimated_sell_price = num('estimated_sell_price') ?? null
      ...
      body.reform_months = num('reform_months') ?? null
  ...
  return (
    <div className={`p-6 max-w-${isFf ? '4xl' : '2xl'} mx-auto`}>
      <div className="flex items-center gap-3 mb-6">
        <Link href={`/dashboard/opportunities/${id}`} className="text-sm text-gray-500 hover:text-gray-700 flex items-center gap-1">
          <ArrowLeft size={14} /> Voltar
        </Link>
        <span className="text-gray-300">|</span>
        <h1 className="text-xl font-bold text-gray-900">
          Editar — {isFf ? 'Fix & Flip' : 'Buy-to-Let'}
        </h1>
      </div>
      ...
      <form onSubmit={handleSubmit}>
        <div className={isFf ? 'grid grid-cols-1 lg:grid-cols-3 gap-4' : 'space-y-4'}>
          ...
          <div className="space-y-4">
            ...
            <Card>
              ...
              <CardContent className="space-y-2">
                ...
                {isFf && <NumInput id="vpt" label="VPT (€)" value={form.vpt} onChange={(v) => set('vpt', v)} hint="Usado no cálculo IMT" />}
            ...
            <div className="space-y-4">
              <Card>
                ...
                <CardContent className="space-y-2">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <NumInput id="electricity_monthly" label="Eletricidade/mês (€)" value={form.electricity_monthly} onChange={(v) => set('electricity_monthly', v)} />
                    <NumInput id="water_monthly" label="Água/mês (€)" value={form.water_monthly} onChange={(v) => set('water_monthly', v)} />
                  </div>
                </CardContent>
              </Card>
              ...
              <Card>
                ...
                <CardContent className="space-y-2">
                  <NumInput id="construction_cost" label="Empreitada (€)" value={form.construction_cost} onChange={(v) => set('construction_cost', v)} />
                  <NumInput id="operational_expenses" label="Despesas Operacionais (€)" value={form.operational_expenses} onChange={(v) => set('operational_expenses', v)} />
              ...
              <Card>
                ...
                <CardContent className="space-y-2">
                  <NumInput id="sale_commission_pct" label="% Comissão imobiliária" value={form.sale_commission_pct} onChange={(v) => set('sale_commission_pct', v)} step="0.1" />
          ...
          {/* ── COLUNA DIREITA (preview + notas + submit) ── */}
          <div className="space-y-4">
            {btlPreview && (
