...
export default async function MarketingPage() {
  ...
  return (
    <div className="p-4 md:p-6 max-w-6xl mx-auto space-y-4 md:space-y-6 overflow-x-hidden">
      <div className="flex items-start justify-between gap-3 flex-wrap">
        <div className="min-w-0">
          <h1 className="text-xl md:text-2xl font-bold text-gray-900">Marketing</h1>
          <p className="text-xs md:text-sm text-gray-500 mt-0.5">Performance das campanhas — últimos 30 dias</p>
        </div>
        ...
        <Card>
          <CardContent className="pt-4 md:pt-5 px-3 md:px-6">
            <div className="flex items-center gap-2 md:gap-3 min-w-0">
              ...
              <div className="min-w-0">
                <div className="text-xl md:text-2xl font-bold truncate">{formatCurrency(totalSpend)}</div>
        ...
        <Card>
          <CardContent className="pt-4 md:pt-5 px-3 md:px-6">
            <div className="flex items-center gap-2 md:gap-3 min-w-0">
              ...
              <div className="min-w-0">
                <div className="text-xl md:text-2xl font-bold truncate">{totalLeads}</div>
        ...
        <Card>
          <CardContent className="pt-4 md:pt-5 px-3 md:px-6">
            <div className="flex items-center gap-2 md:gap-3 min-w-0">
              ...
              <div className="min-w-0">
                <div className="text-xl md:text-2xl font-bold truncate">{formatCurrency(avgCPL)}</div>
        ...
        <Card>
          <CardContent className="pt-4 md:pt-5 px-3 md:px-6">
            <div className="flex items-center gap-2 md:gap-3 min-w-0">
              ...
              <div className="min-w-0">
                <div className="text-xl md:text-2xl font-bold truncate">{campaigns.length}</div>
            ...
            {campaigns.length === 0 ? (
              ...
              <div className="space-y-3">
                {campaigns.map((campaign) => {
                  ...
                  return (
                    <div key={campaign.id} className="p-3 md:p-4 border border-gray-100 rounded-lg overflow-hidden">
                      <div className="flex items-start justify-between gap-2 mb-2">
                        <div className="min-w-0 flex-1">
                          <p className="font-medium text-sm text-gray-800 break-words">{campaign.name}</p>
                        ...
                        {cpl && (
                          <div className="text-right flex-shrink-0">
                            <div className="text-base md:text-lg font-bold text-gray-700">{formatCurrency(cpl)}</div>
                            <div className="text-xs text-gray-400">CPL</div>
                          </div>
                        )}
                      </div>
                      {m && (
                        <div className="grid grid-cols-4 gap-1 md:gap-2 text-center pt-2 border-t border-gray-100">
                          <div className="min-w-0">
                            <div className="text-xs md:text-sm font-semibold truncate">{formatCurrency(m.spend)}</div>
                            <div className="text-xs text-gray-400">Gasto</div>
                          </div>
                          <div className="min-w-0">
                            <div className="text-xs md:text-sm font-semibold truncate">{m.impressions.toLocaleString('pt-PT')}</div>
                            <div className="text-xs text-gray-400">Impr.</div>
                          </div>
                          <div className="min-w-0">
                            <div className="text-xs md:text-sm font-semibold truncate">{m.leads}</div>
